...
VISION_ACTIVE = False  # Default to False: Safe Mode
...
# SMOOTHNESS TWEAK: Reduced from 2.5s to 1.0s to feel less "clunky"
...
# Note: 'func' is only called if VISION_ACTIVE is True (except for activation gestures)
GESTURE_ACTIONS = {
    "POINTING_INDEX": {
        "desc": "Open Up last Closed Tab",
        "func": lambda: pyautogui.hotkey('command', 'shift', 't')
    },
    "PEACE": {
        "desc": "New Tab (Cmd+T)",
        "func": lambda: pyautogui.hotkey('command', 't')
    },
    "SHAKA": {
        "desc": "Close Tab (Cmd+W)",
        "func": lambda: pyautogui.hotkey('command', 'w')
    }
}
...
def prune_history(messages, max_turns=5): 
    t_start = time.time()
    pruned = []
    if len(messages) > 0: pruned.append(messages[0])
    recent_messages = messages[1:]
    if len(recent_messages) > max_turns: recent_messages = recent_messages[-max_turns:]
        ...
        if isinstance(msg["content"], list):
            for block in msg["content"]:
                if isinstance(block, dict) and block.get("type") == "image":
                    if msg == messages[-1]: new_content.append(block)
                    else: new_content.append({"type": "text", "text": "[Image Pruned]"})
                    ...
                    if isinstance(inner_content, list):
                        for inner_block in inner_content:
                            if isinstance(inner_block, dict) and inner_block.get("type") == "image":
                                if msg == messages[-1]: new_inner_content.append(inner_block)
                                else: new_inner_content.append({"type": "text", "text": "[Screenshot Pruned]"})
                            else: new_inner_content.append(inner_block)
                    new_tool_result["content"] = new_inner_content
                    new_content.append(new_tool_result)
                else: new_content.append(block)
        else: new_content = msg["content"]
    ...
    while True:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                if e.status_code == 429: time.sleep(15 * (attempt + 1))
                else: raise e
        ...
        if response.stop_reason == "tool_use":
            ...
            for block in response.content:
                if block.type == "tool_use":
                    ...
                    else: output_content = [{"type": "text", "text": "Action executed."}]
                    ...
                    if volume > self.threshold:
                        ...
                        if time.time() - silence_start > self.silence_limit:
                            ...
                            segments, _ = self.model.transcribe(
                                audio_np, beam_size=5, vad_filter=True, condition_on_previous_text=False,
                                initial_prompt="Nova command context."
                            )
...
def main_loop():
    ...
    print("Agent is Ready. Say 'Nova [command]' or show 'CLOSED FIST' to activate vision.")
    
    while True:
        try:
            # 1. PROCESS GESTURES
            try:
                gesture_name = vision_stream.gesture_queue.get_nowait()
                current_time = time.time()
                
                # ACTIVATION / DEACTIVATION LOGIC (Ignores Cooldown)
                if gesture_name == "CLOSED_FIST":
                    if not VISION_ACTIVE:
                        VISION_ACTIVE = True
                        print("\n" + "="*40)
                        print("VISION MODE ACTIVATED (Ready for commands)")
                        print("="*40 + "\n")
                        subprocess.run(["afplay", "/System/Library/Sounds/Hero.aiff"])
                        last_gesture_time = current_time # Reset cooldown so we don't trigger immediately
                    continue
                ...
                # COMMAND LOGIC (Only if Active + Cooldown passed)
                if VISION_ACTIVE and (current_time - last_gesture_time > GESTURE_COOLDOWN):
                    if gesture_name in GESTURE_ACTIONS:
                        action_details = GESTURE_ACTIONS[gesture_name]
                        
                        print("\n" + "="*40)
                        print(f"VISION DETECTED: {gesture_name}")
                        print(f"ACTION: {action_details['desc']}")
                        print("="*40 + "\n")
    ...
    while not STOP_AGENT.is_set():
        try:
            ...
            if "nova" in text or "stop" in text or "wait" in text:
                ...
                if command_part: PENDING_COMMAND = command_part
                STOP_AGENT.set()
                subprocess.run(["afplay", "/System/Library/Sounds/Submarine.aiff"])
        except queue.Empty:
            time.sleep(0.1)
